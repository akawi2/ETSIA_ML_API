...
METRICS_ENDPOINT = f"{SUPABASE_URL}/functions/v1/evaluate-alerts"

def emit_metric(service: str, event_name: str, params: dict, model: str = None):
    """Envoie une métrique vers Supabase Edge Function pour évaluation."""
    try:
        payload = {
            "service": service,
        ...
        # Timeout court pour ne pas ralentir l'app métier
        requests.post(METRICS_ENDPOINT, json=payload, headers=headers, timeout=0.5)
...
def predict_depression(model_type: str = "camembert"):
    ...
    emit_metric(
        ...
        params={
            ...
            "ram_usage": random.choice([500, 2100]),
...
def generate_content():
    ...
    emit_metric("content_generation", "generate_post", metrics, model="llama3.2:3b")
